...
- integrated_multimodal_description: 只含本段的一个 [Shot 1] 块，英文；正文以 `[Shot 1] ` 标记紧跟字段名开头
  ...
  **机位与节拍二选一**：快机位（fast/rapid tracking、whip pan、`at fast speed` 等）与 ≥3 个动作节拍
  （正文 ≥3 个 `At 00:XX.XXX`）不得同段共存，否则产出剧烈闪动——**注意这条阈值量于 4 步采样**
  （同 seed 实测 4.1–4.9），同一文本在 8 步采样下只有 0.869、并不闪：它是**降级采样配置下的
  现象**，不是内容的固有性质。纪律照旧执行（4 步仍会出现；要改采样配置须先重测）。默认降机位为
  静态/慢速中景、节拍照写，剧情必须快镜时把本段节拍压到 2 个以内；
...
1. `integrated_multimodal_description:` 本段剧情，**默认只有 [Shot 1] 一个镜头块**（段内切镜是显式例外，仅景别跳变等确有必要时）。
   ...
   - **机位与节拍二选一（实测纪律）**：快机位（fast/rapid tracking、whip pan、`at fast speed` 等）
     ...
     产出剧烈闪动的片段。**这条阈值量于 4 步采样**（同 seed 实测 4.1–4.9），而同一文本在
     **8 步**采样下只有 0.869、并不闪：它是**降级采样配置下的现象**，不是内容的固有性质。
     纪律照旧执行（4 步仍会出现；要改采样配置须先重测）。默认**降机位**：改静态/慢速中景
