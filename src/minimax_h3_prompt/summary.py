"""最终 H3 提示词的中文摘要：让用户不读英文也能核对视频走向。

阶段 2 产出完整英文提示词后，追加一次轻量 LLM 调用，把整条提示词翻译提炼为
中文摘要：整体剧情走向（3-5 句）+ 每个 Shot 一句画面、一句声音。摘要是纯展示层，
...
_SUMMARY_INSTRUCTION = """你是中文视频内容解说员。下面是一条 H3 视频生成提示词（英文）。
