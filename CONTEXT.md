...
**用户修订（user revision）**:
用户在阶段 1 展示首帧生图提示词后提出的修改要求，**按轮累积**（人说的，说了就不忘）。每条带一个**层次**，且层次＝重跑起点：**画面级修订**只重跑首帧提示词节点（＝只改画面，代价与逐帧重出相同）；**设定级修订**回灌对应产物（人物设计／背景设计／道具设计／剧本／分镜表）并**重跑该层及其下游**，故它可能**替换用户已经看过的产物**（重跑前先告知）。层次由用户自己声明——只有人知道这条是画面口味还是设定变更。落 `state["user_revisions"]`。
