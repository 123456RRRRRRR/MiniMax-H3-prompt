...
**用户修订（user revision）**:
用户在阶段 1 展示首帧生图提示词后提出的修改要求，**按轮累积**（人说的，说了就不忘）。每条带一个**层次**，且层次＝重跑起点：**画面级修订**只重跑首帧提示词节点（＝只改画面，代价与逐帧重出相同）；**设定级修订**回灌对应产物（人物设计／背景设计／道具设计／剧本／分镜表）并**重跑该层及其下游**，故它可能**替换用户已经看过的产物**（重跑前先告知）。层次由用户自己声明——只有人知道这条是画面口味还是设定变更（四档：只改画面／人物设定／美术场景道具／剧情分镜，层次＝**重跑起点**，不另造映射）。落 `state["user_revisions"]`。
设定级重跑**成功后**该条标**已落地**，此后各注入通道（首帧节点、分段写段、分段规划）都不再带它——它的要求已经写进设定产物本身，再带一遍就是同一条要求叠加第二次；重跑失败或中断则**不标**（清单不得谎报）。已落地的条目仍留在清单里（屏幕单列一段，逐条标「已落地，不再注入」），但不占**撤销编号**：编号是「模型看到的第几条」与「撤销要念的号」共用的那一套，占进去就会让同一个号指向两条不同的话。
...
**撤销（revoke）**:
按**编号**撤掉累积清单里的**任意一条**（不只末尾那条——被后来意见覆盖的旧条也常需要撤），撤掉一条后其余条重排序。编号是**屏幕清单的位置号**，与注入块的编号同源，所以「我撤的是模型看到的第 2 条吗」有确定答案。撤销**也是一轮**：照样重出、照样留台账行。
撤销**不止是摘清单**：本轮要显式告知模型「这几条不再生效、其造成的画面改动要撤回」——只摘条目的话，**修订基线**那句「未提到的部分原样保留」会把上一版里已落地的改动原样保住，撤销成空动作。清单撤空后重出仍合法（等于不带任何修订）。
_Avoid_: 把撤销当成「删除记录」（它不改历史台账，只改本轮起生效的清单）、把编号当轮次号
