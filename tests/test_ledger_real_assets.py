...
def ledger():
    """一次算完，5 个用例共用——每次 build_ledger 要读真实 mp4 做窗口比对，约 9 秒。"""
    return build_ledger(SESSION, [COMFY_OUTPUT])


def test_recovers_five_shots_in_correct_order(ledger):
...
def test_reports_plan_expects_six_but_five_produced(ledger):
